...
!function (a, b, c, d) {
      ...
      init: function () {
        h.transition = h.transition(), h.build(), h.options.target ? "number" == typeof h.options.target ? h.target = h.options.target : "string" == typeof h.options.target && (h.target = Math.floor(c(h.options.target).offset().top)) : h.target = 0, h.$trigger.on("click.scrollToTop", function () {
          ...
          if (!h.disabled) {
            h.checkMobile();
            var b, d;
            if (h.useMobile ? (b = h.options.mobile.speed, d = h.options.mobile.easing):(b = h.options.speed, d = h.options.easing), h.$doc.addClass(h.classes.animating), !h.transition.supported)return void c("html, body").stop(!0, !1).animate({scrollTop: h.target}, b, function () {
              h.$doc.removeClass(h.classes.animating)
            });
            var e = c(a).scrollTop();
            h.$doc.css({"margin-top": -e + h.target + "px"}), c(a).scrollTop(h.target), h.insertRule(".duration_" + b + "{" + h.transition.prefix + "transition-duration: " + b + "ms;}"), h.$doc.addClass("easing_" + d + " duration_" + b).css({"margin-top": ""}).one(h.transition.end, function () {
        ...
        return /(WebKit)/i.test(a.navigator.userAgent) && (f = "-webkit-"), {prefix: f, end: e, supported: g}
      }, insertRule: function (a) {
        if (!h.rules || !h.rules[a])if (h.rules === d ? h.rules = {} : h.rules[a] = !0, b.styleSheets && b.styleSheets.length)b.styleSheets[0].insertRule(a, 0); else {
  ...
  e.defaults = {
    ...
    if ("string" == typeof a) {
      var b = a, f = arguments.length > 1 ? Array.prototype.slice.call(arguments, 1) : d;
